...
export default function HomePage() {
  ...
  const [latestMoviesFilter, setLatestMoviesFilter] = useState<'All' | 'Action' | 'Comedy' | 'Horror'>('All');
  const [latestSeriesFilter, setLatestSeriesFilter] = useState<'All' | 'Action' | 'Comedy' | 'Sci-Fi'>('All');
  const [selectedCountry, setSelectedCountry] = useState('All');
  const [selectedLetter, setSelectedLetter] = useState('All');

  // Supabase live movies
  const [supabaseMovies, setSupabaseMovies] = useState<DisplayMovie[]>([]);
  ...
  // Combine live movies with catalog
  const heroMovie = useMemo(() => {
    if (supabaseMovies.length > 0) {
      const pinned = supabaseMovies.find((m) => m.type === 'movie');
      if (pinned) return pinned;
    }
    return NEXT_HERO_MOVIE;
  }, [supabaseMovies]);

  const allMoviesList = useMemo(() => {
    const list = [...supabaseMovies, ...NEXT_SUGGESTIONS, ...NEXT_LATEST_MOVIES, ...NEXT_LATEST_SERIES];
  ...
  const suggestionsList = useMemo(() => {
    const liveSuggestions = supabaseMovies.filter((m) => m.type === 'movie');
    const merged = [...liveSuggestions, ...NEXT_SUGGESTIONS];
    if (suggestionsFilter === 'Recommended') {
      return [...merged].reverse().slice(0, 16);
    }
    return merged.slice(0, 16);
  }, [supabaseMovies, suggestionsFilter]);

  const latestMoviesList = useMemo(() => {
    const liveMovies = supabaseMovies.filter((m) => m.type === 'movie');
    const merged = [...liveMovies, ...NEXT_LATEST_MOVIES];
    if (latestMoviesFilter === 'All') return merged.slice(0, 16);
    return merged
      .filter((m) => m.genre.toLowerCase().includes(latestMoviesFilter.toLowerCase()))
      .slice(0, 16);
  }, [supabaseMovies, latestMoviesFilter]);

  const latestSeriesList = useMemo(() => {
    const liveSeries = supabaseMovies.filter((m) => m.type === 'series');
    const merged = [...liveSeries, ...NEXT_LATEST_SERIES];
    if (latestSeriesFilter === 'All') return merged.slice(0, 16);
    return merged
      .filter((m) => m.genre.toLowerCase().includes(latestSeriesFilter.toLowerCase()))
      .slice(0, 16);
  }, [supabaseMovies, latestSeriesFilter]);
        ...
        {searchQuery.trim() ? (
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between bg-white px-4 py-3 rounded-xl border border-gray-200 shadow-2xs">
              ...
              <div className="flex items-center gap-1 text-xs font-medium">
                {(['All', 'Action', 'Comedy', 'Sci-Fi'] as const).map((filter) => (
                  <button
                    key={filter}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between bg-white px-4 py-3 rounded-xl border border-gray-200 shadow-2xs">
              ...
              <div className="flex items-center gap-1 text-xs font-medium">
                {(['All', 'Action', 'Comedy', 'Horror'] as const).map((genre) => (
                  <button
                    key={genre}
            ...
            <section className="space-y-3 pt-2 pb-6">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                  ...
                  <div className="flex items-center gap-1 text-xs font-medium">
                    {(['All', 'Action', 'Comedy', 'Sci-Fi'] as const).map((genre) => (
                      <button
                        key={genre}
